...
func generateMarkdownReport(result S4RemediationResult) string {
	...
	if result.Artifact.TransportRequest != "" {
		fmt.Fprintf(&sb, "| **Transport Request** | %s |\n", result.Artifact.TransportRequest)
	}
	sb.WriteString("\n")
	...
	if len(result.Issues) == 0 {
		...
		fmt.Fprintf(&sb, "**Total Issues Found: %d**\n\n", len(result.Issues))
	...
	if len(result.Issues) > 0 {
		...
		for i, issue := range result.Issues {
			...
			fmt.Fprintf(&sb, "**Symptom Detected:** `%s`\n\n", issue.SymptomDetected)
			...
			fmt.Fprintf(&sb, "**Fix Description:** %s\n\n", issue.FixDescription)
			sb.WriteString("---\n\n")
		}
	}
	...
	sb.WriteString("---\n")
	fmt.Fprintf(&sb, "*Generated by ABAPER MCP - %s*\n", result.RunMetadata.TimestampUTC)
